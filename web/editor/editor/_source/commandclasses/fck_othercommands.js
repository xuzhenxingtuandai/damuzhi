...
/*phpox begin*/
/*var FCKPBoxCommand=function()
{
    this.Name='PBox';
}

FCKPBoxCommand.prototype=
{
    Execute:function()
    {
    	parent.window.windowsdig('选择文件','iframe:index.php?case=file&act=updialog&fileinputid='+FCK.Name+'&maxselect=13&checkfrom=edit','900px','480px','iframe');
    },
    GetState:function()
    {
        if ( FCK.EditMode != FCK_EDITMODE_WYSIWYG )
            return FCK_TRISTATE_DISABLED;
        return FCK.GetNamedCommandState('OpenPBox');
    }
};
*/
...
FCKDelPicCommand.prototype =
{
    Execute : function()
    {
        ...
        var sUrl = encodeURIComponent(oImage.getAttribute('_fcksavedurl')); // 得到图片存储地址
        ...
        if (confirm('要删除服务器上的图片吗？')) {
            xml.open("get", "../../index.php?case=file&act=delfile&dfile=" + sUrl + "&UD=1", false);
