...
class table_admin extends admin {
    ...
    function upload_action(){
        if($_SERVER['REQUEST_METHOD']=="POST") {
           if($_POST['type']=='clear') {
               $file = ROOT ."/js/bulkupload/php/log.txt";
               $result = @unlink ($file);
               if ($result == true) {
                   echo -200;
                   die;
                   ...
                   $str='';
                   $open = fopen(ROOT . "/js/bulkupload/php/log.txt", "w");
                   fwrite($open, $str);
                   fclose($open);
                   echo -200;
                   die;
                ...
                $file_name = ROOT ."/js/bulkupload/php/log.txt";
                if( is_file( $file_name ) )
                {
                    $cn = file_get_contents( $file_name );
                    $temp_arr = explode("|", $cn);
                    for($i=0;$i<count($temp_arr)-1;$i++) {
                        $title_arr = explode(",", $temp_arr[$i]);

                        $this->manage->filter($this->Exc);
                        $this->manage->add_before($this);
                        $this->manage->save_before();

                        front::$post["catid"] = $_POST["catid"];
                        front::$post["typeid"] = $_POST["typeid"];
                        front::$post["title"] = $title_arr[0];
                        front::$post["tar"] = '';
                        front::$post["color"] = '';
                        front::$post["font"] = '';
                        front::$post["spid"] = 0;
                        front::$post["ip"] = '';
                        front::$post["mtitle"] = $title_arr[0];
                        front::$post["keyword"] = '';
                        front::$post["description"] = '';
                        front::$post["listorder"] = '';
                        front::$post["image"] = $title_arr[1];
                        front::$post["thumb"] = $title_arr[1];
                        front::$post["introduce"] = '内容';
                        front::$post["content"] ='';
                        front::$post["template"] = $_POST["template"];
                        front::$post["showform"] = '';
                        front::$post["htmlrule"] = '';
                        front::$post["linkto"] = '';
                        front::$post["attr2"] = '';
                        front::$post["attr3"] = '';
                        front::$post["attachment_id"] = '';
                        front::$post["attachment_path"] = '';
                        front::$post["type"] = '';
